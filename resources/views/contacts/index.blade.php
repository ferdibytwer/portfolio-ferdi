    ...
    <section id="contact">
        <div class="container_">
            <div class="mt-20 mb-40">
                <h1 class="text-subheading text-center">Contact</h1>
                <p class="text-center mb-8">Hubungi saya untuk pemesanan layanan atau kolaborasi lebih lanjut. Saya
                    senang berkolaborasi dengan lebih banyak orang.</p>
                <div class="flex gap-x-8 flex-col sm:flex-row gap-y-8">
                    {{-- Left --}}
                    @if (session('success'))
                        <div id="success-alert"
                            class="fixed top-5 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 transition-opacity duration-500">
                            {{ session('success') }}
                        </div>
                    @endif
                    <form method="POST" action="{{ route('contact.send') }}"
                        class="flex-1 sm:flex-2/3 md:flex-1/2 space-y-4">
                        @csrf

                        <input type="text" name="name" placeholder="Nama" required
                            class="w-full px-4 py-2 border  rounded-md focus:outline-none focus:ring-0 transition-none">

                        <input type="email" name="email" placeholder="Email" required
                            class="w-full px-4 py-2 border  rounded-md focus:outline-none focus:ring-0 transition-none">

                        <textarea name="message" rows="4" placeholder="Pesan" required
                            class="w-full px-4 py-2 border  rounded-md focus:outline-none focus:ring-0 transition-none"></textarea>

                        <button type="submit"
                            class="w-full text-white px-4 py-2 border border-black shadow-sm bg-black group flex items-center justify-center">
                            Kirim pesan
                        </button>
                    </form>
                    {{-- Right --}}
                    <div class="flex-1 sm:flex-1/3 md:flex-1/2">
                        <div class="flex flex-col gap-y-4">
                            <h2 class="font-medium md:text-lg text-center sm:text-left">Ikuti saya di sosial media</h2>
                            <div class="flex flex-col gap-y-4">
                                {{-- WhatsApp --}}
                                <x-button href="https://wa.me/6283850147419" target="_blank">
                                ...
                                {{-- Instagram --}}
                                <x-button href="https://www.instagram.com/_z1729" target="_blank">
                                    <div class="flex flex-row items-center space-x-2">
                                ...
                                {{-- LinkedIn --}}
                                <x-button href="https://www.linkedin.com/in/bayu-ferdianto1729" target="_blank">
                                ...
                                {{-- Github --}}
                                <x-button href="https://github.com/ferdibytwer" target="_blank">
                                    <div class="flex flex-row items-center space-x-2">
